    ...
    for j=1:N
        ...
        for i=1:L-1                    %光伏
            if (Cs(j,2*k-1)>=pvs{1,k+6}(i,2))&(Cs(j,2*k-1)<pvs{1,k+6}(i+1,2))
             ...
             s2=subs(s1,{'x'},{Cs(j,2*k-1)});
             cPpvs1(j)=eval(s2);
            end
        end
        for i=1:L1-1                    %风机        
            if (Cs(j,2*k)>=wts{1,k+6}(i,2))&(Cs(j,2*k)<wts{1,k+6}(i+1,2))
            s1=subs(s,{'a(i)','coe(i,1)','coe(i,2)','coe(i,3)','coe(i,4)'},{awts{1,k+6}(i),coewts{1,k+6}(i,1),coewts{1,k+6}(i,2),coewts{1,k+6}(i,3),coewts{1,k+6}(i,4)});
            s2=subs(s1,{'x'},{Cs(j,2*k)});
            cPwts1(j)=eval(s2);
            end
        end
    end
    ...
    for j=1:N
        ...
        for i=1:L-1                    %光伏
            if (Cinter(j,2*k-1)>=pvinter{1,k+6}(i,2))&(Cinter(j,2*k-1)<pvinter{1,k+6}(i+1,2))
             ...
             s2=subs(s1,{'x'},{Cinter(j,2*k-1)});
             cPpvinter1(j)=eval(s2);
            end
        end
        for i=1:L1-1                    %风机        
            if (Cinter(j,2*k)>=wtinter{1,k+6}(i,2))&(Cinter(j,2*k)<wtinter{1,k+6}(i+1,2))
            s1=subs(s,{'a(i)','coe(i,1)','coe(i,2)','coe(i,3)','coe(i,4)'},{awtinter{1,k+6}(i),coewtinter{1,k+6}(i,1),coewtinter{1,k+6}(i,2),coewtinter{1,k+6}(i,3),coewtinter{1,k+6}(i,4)});
            s2=subs(s1,{'x'},{Cinter(j,2*k)});
    ...
    for j=1:N
        ...
        for i=1:L-1                    %光伏
            if (Cw(j,2*k-1)>=pvw{1,k+6}(i,2))&(Cw(j,2*k-1)<pvw{1,k+6}(i+1,2))
             ...
             s2=subs(s1,{'x'},{Cw(j,2*k-1)});
             cPpvw1(j)=eval(s2);
            end
        end
        for i=1:L1-1                    %风机        
            if (Cw(j,2*k)>=wtw{1,k+6}(i,2))&(Cw(j,2*k)<wtw{1,k+6}(i+1,2))
            s1=subs(s,{'a(i)','coe(i,1)','coe(i,2)','coe(i,3)','coe(i,4)'},{awtw{1,k+6}(i),coewtw{1,k+6}(i,1),coewtw{1,k+6}(i,2),coewtw{1,k+6}(i,3),coewtw{1,k+6}(i,4)});
            s2=subs(s1,{'x'},{Cw(j,2*k)});
            cPwtw1(j)=eval(s2);
            end
        end
    end
